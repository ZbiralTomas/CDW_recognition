...
def calibrate_camera(image_list, checkerboard_size=(9, 7)):
    ...
    # Loop through your captured checkerboard images
...
while True:
    ...
    # Slečna Terezka - tady jsou data už načtená
        ...
        decoded_image = cv2.undistort(decoded_image, cam_matrix, distortion_coeffs)
        if rotation:
        ...
        while len(chessboard_list) < 15:
            ...
            cv2.imshow('Calibration', decoded_image)
            key = cv2.waitKey(1) & 0xFF
            if key == ord(' '):
                # Save the current frame as an image and append it to the chessboard list
                chessboard_list.append(frame.copy())
                print('Image saved.')

    cam_matrix, distortion_coeffs = calibrate_camera(chessboard_list)
